...
-- ── Layering plan ──────────────────────────────────────────────────────────
-- ZIndex 0  : LineHighlight frame (behind everything)
-- ZIndex 2  : SelectionHighlight frame
-- ZIndex 3  : SynBase  — FULL text in default colour (catches every plain identifier)
-- ZIndex 5  : Syntax colour layers (paint OVER SynBase for keywords/strings/etc.)
--             Each layer replaces non-matching chars with spaces so only its tokens show.
-- ZIndex 6  : Caret frame (above all text)
-- CodeBox itself is transparent — it only captures input & provides the cursor.
-- ───────────────────────────────────────────────────────────────────────────

-- Base layer: always renders the full source in default colour
G.SynBase = Instance.new("TextLabel")
G.SynBase.Name               = "Base_"
G.SynBase.Size               = UDim2.new(1, 0, 1, 0)
G.SynBase.Position           = UDim2.new(0, 0, 0, 0)
G.SynBase.BackgroundTransparency = 1
G.SynBase.BorderSizePixel    = 0
G.SynBase.Text               = ""
G.SynBase.FontFace           = FONT_MONO
G.SynBase.TextSize           = 14
G.SynBase.TextColor3         = C.TextNormal   -- plain-text fallback colour
G.SynBase.TextXAlignment     = Enum.TextXAlignment.Left
G.SynBase.TextYAlignment     = Enum.TextYAlignment.Top
G.SynBase.ZIndex             = 3
G.SynBase.Parent             = G.CodeBox

-- Colour layers that overlay specific token types
local function makeSynLabel(name, color)
	...
	lbl.ZIndex             = 5   -- above SynBase (3), below caret (6)
	lbl.Parent             = G.CodeBox
	return lbl
end

G.SynComments = makeSynLabel("Comments_",        C.SynComment)
G.SynComments.ZIndex = 7   -- comments win over every other syntax layer
...
-- No SynTokens needed — SynBase already renders all plain text
...
local function updateHighlighting()
	...
	if G.CodeBox.Text ~= s then G.CodeBox.Text = s end

	-- Base layer always shows the full source so plain identifiers are never invisible
	G.SynBase.Text     = s

	-- Colour layers: each replaces non-matching chars with spaces and paints over SynBase
	...
	G.SynComments.Text = highlightComments(s)  -- highest priority — overwrites everything
